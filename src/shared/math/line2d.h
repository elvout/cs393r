...
struct Line {
  ...
  T Length() const { return (p0 - p1).norm(); }

  T SqLength() const { return (p0 - p1).squaredNorm(); }

  Vector2T Dir() const { return (p1 - p0).normalized(); }
  ...
  T ClosestApproach(const Line<T>& l) const { return ClosestApproach(l.p0, l.p1); }
  ...
  bool Crosses(const Vector2T& p2, const Vector2T& p3) const {
    ...
    if (std::min(p0.x(), p1.x()) > std::max(p2.x(), p3.x()))
      return false;
    if (std::max(p0.x(), p1.x()) < std::min(p2.x(), p3.x()))
      return false;
    if (std::min(p0.y(), p1.y()) > std::max(p2.y(), p3.y()))
      return false;
    if (std::max(p0.y(), p1.y()) < std::min(p2.y(), p3.y()))
      return false;
    // Narrow-phase check.
  ...
  bool Intersects(const Vector2T& p2, const Vector2T& p3) const {
    ...
    const Vector2T d1 = p1 - p0;
    const Vector2T d2 = p3 - p2;
    if (Cross<T>(d1, p3 - p0) * Cross<T>(d1, p2 - p0) > 0.0)
      return false;
    if (Cross<T>(d2, p1 - p2) * Cross<T>(d2, p0 - p2) > 0.0)
      return false;
    // Okay, the line segments definitely intersect.
    return true;
  }
  ...
  bool Intersects(const Line<T>& l2) const { return Intersects(l2.p0, l2.p1); }

  bool Crosses(const Line<T>& l2) const { return Crosses(l2.p0, l2.p1); }
